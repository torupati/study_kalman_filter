...
from .ekf import EkfConfig, IDX_BG, IDX_BAX, IDX_BAY, IDX_VX, IDX_VY, IDX_X, IDX_Y, IDX_YAW, run_filter, wrap_angle
from .plotting import create_summary_figure, save_figure
...
def summarise_errors(truth_states: np.ndarray, state_estimates: np.ndarray) -> dict[str, float]:
    ...
    yaw_rmse_deg = np.rad2deg(
        np.sqrt(np.mean(wrap_angle(state_estimates[:, IDX_YAW] - truth_states[:, IDX_YAW]) ** 2))
    )
